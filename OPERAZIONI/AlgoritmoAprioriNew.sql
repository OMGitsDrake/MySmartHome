...
BEGIN
    ...
    /*
    SELECT (16/100)*COUNT(*) INTO @valoreSoglia						-- si inserisce il valore di soglia per il pruning
    FROM TransazioniTarget;
    */
    
    SET @valoreSoglia = 9; -- più alto => meno membri a sinistra
    -- SELECT @valoreSoglia;
	...
	ELSE 
		SELECT 'Non è possibile stabilire delle regole di associazione';
	END IF;
END $$
...
-- -------------------------------------------------------------------------------------
-- creazione della procedura di Pruning, sfoltisce gli items non considerati ricorrenti
-- -------------------------------------------------------------------------------------
