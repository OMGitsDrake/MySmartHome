...
BEGIN
    ...
    /*
		Dalla query sottostante  prenderemo tutti i dispositivo a ciclo non interrompibile con i vari
        livelli
    */
	...
	(
		...
		WHERE MONTH(SPE.InizioTimeStamp) = MONTH(CURRENT_TIMESTAMP)
			AND DAY(SPE.InizioTimeStamp) BETWEEN (DAY(CURRENT_TIMESTAMP - INTERVAL 7 DAY) - 3) AND (DAY(CURRENT_TIMESTAMP - INTERVAL 7 DAY) + (7 - 3))
    ...
    -- per ogni dispositivo con un determinato livello e durata
        ...
        -- stimo quanti KW mi rimangono da produzione - consumo
