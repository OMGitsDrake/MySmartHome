DROP PROCEDURE IF EXISTS StimaConsumoImpostazione;

DELIMITER $$

CREATE PROCEDURE StimaConsumoImpostazione(_IDImpostazione INT, _Giorno INT, _mese INT)
BEGIN
    ...
    -- in caso in cui si voglia fare una stima nel futuro
		...
		(
			...
			WHERE MONTH(T.TimeStampRilevazione) = 9 -- _mese
				AND DAY(T.TimeStampRilevazione) = 5 -- _Giorno
				AND T.IDStanza = 11 -- _IDStanza
        ...
        INTO TemperaturaInterna, MediaErroreTemperaturaInterna, TemperaturaEsterna, MediaErroreTemperaturaEsterna, ConsumoKW;
    ...
    -- in caso sia un giorno del passato dovremo solo attenersi ai dati memorizzati
        ...
        WHERE YEAR(T.TimeStampRilevazione) = YEAR(CURRENT_DATE)
			AND MONTH(T.TimeStampRilevazione) = 9 -- _mese
            AND DAY(T.TimeStampRilevazione) = 5 -- _Giorno
            AND T.IDStanza = 11; -- _IDStanza;
    END IF;
    
    IF MediaErroreTemperaturaInterna IS NULL OR MediaErroreTemperaturaEsterna IS NULL THEN
		SELECT CONCAT('La temperatura interna era di ', TemperaturaInterna, ', mentre la temperatura esterna ', TemperaturaEsterna, ' consumando ', ConsumoKW) INTO Risultato;
    ELSE
		SELECT CONCAT('Si presume una temperatura interna di ', TemperaturaInterna, ', una temperatura esterna di ', TemperaturaEsterna, ' +/-', (MediaErroreTemperaturaInterna+MediaErroreTemperaturaEsterna)/2, ' e un consumo di ', ConsumoKW + ConsumoTotaleDispositivoLivello) INTO Risultato;
    END IF;
    
    IF Risultato IS NULL THEN
		SELECT "ERRORE";
		SELECT CONCAT_WS('', 'Si presume una temperatura interna di ', TemperaturaInterna, ', una temperatura esterna di ', TemperaturaEsterna, ' +/-', MediaErroreTemperaturaEsterna, ' e un consumo di ', ConsumoKW + ConsumoTotaleDispositivoLivello);
	ELSE
		SELECT Risultato;
	END IF;
END $$
